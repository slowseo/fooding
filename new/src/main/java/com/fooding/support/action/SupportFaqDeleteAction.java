...
public class SupportFaqDeleteAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t M : SupportFaqDeleteAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int result = dao.supportFaqDelete(table_id);
		
		if(result == 1) {
			JSMoveFunction.alertLocation(response, "FAQ가 삭제되었습니다.", "./SupportFaqList.sup?pageNum=" + pageNum);
			return null;
		}else {
