...
public class SupportNoticeUpdateProAction implements Action {
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int result = dao.supportNoticeUpdate(dto);
			
		// 페이지 이동
		if(result == 1) {
			JSMoveFunction.alertLocation(response, "공지사항이 수정되었습니다.", "./SupportNoticeContent.sup?table_id=" + dto.getTable_id() + "&pageNum=" + pageNum);
			return null;
		}else {
