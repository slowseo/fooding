...
public class SupportFaqListAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t M : SupportFaqList_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		ArrayList list = null;
		if(count > 0) {
			list = dao.supportFaqList(startRow, pageSize);
		}
		...
		// 페이지 이동
		forward.setPath("./fooding/khr/support/faqList.jsp");
