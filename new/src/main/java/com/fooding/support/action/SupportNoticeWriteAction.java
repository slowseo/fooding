...
public class SupportNoticeWriteAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t M : SupportFaqWriteAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		dao.supportNoticeInsert(dto);
		
		// 페이지 이동
		forward.setPath("./SupportNoticeList.sup");
		forward.setRedirect(true);
		return forward;
	}
}
