...
public class SupportEventUpdateProAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t M : SupportEventUpdateAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int result = dao.supportEventUpdate(dto);
			
		// 페이지 이동
		if(result == 1) {
			JSMoveFunction.alertLocation(response, "이벤트가 수정되었습니다.", "./SupportEventContent.sup?table_id=" + dto.getTable_id() + "&pageNum=" + pageNum);
			return null;
		}else {
