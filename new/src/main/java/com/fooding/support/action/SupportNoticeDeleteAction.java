...
public class SupportNoticeDeleteAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t M : SupportNoticeDeleteAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int result = dao.supportNoticeDelete(table_id);
		
		if(result == 1) {
			JSMoveFunction.alertLocation(response, "공지사항이 삭제되었습니다.", "./SupportNoticeList.sup?pageNum=" + pageNum);
			return null;
		}else {
