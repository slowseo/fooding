...
public class AdminPurchaseListAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t\t M : AdminPurchaseListAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int count = dao.adminPurchaseCount();
		...
		ArrayList list = null;
		if (count > 0) {
			list = dao.adminPurchaseList(startRow, pageSize);
		}
		...
		// 페이지 이동
		forward.setPath("./fooding/khr/admin/adminPurchase.jsp");
