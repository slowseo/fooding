...
public class ProfileFoodtruckListAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t\t M : ProfileFoodtruckListAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int member_id = pdao.profileMember_id(id);
		System.out.println("member_id : " + member_id);

		// 카운트 계산
		ProfileDAO dao = new ProfileDAO();
		int count = dao.profileFoodtruckCount(member_id);
		...
		ArrayList list = null;
		if (count > 0) {
			list = dao.profileFoodtruckList(member_id, startRow, pageSize);
		}
		...
		// 페이지 이동
		forward.setPath("./fooding/khr/profile/foodtruckList.jsp");
