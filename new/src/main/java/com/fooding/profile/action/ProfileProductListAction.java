...
public class ProfileProductListAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 출력문
		System.out.println("\t\t M : ProfileProductListAction_execute() 호출 ");
...
//		session.setAttribute("id", "admin"); // 이 부분만 삭제하면 됨
		...
		int count = dao.profileProductCount(member_id);
		...
		ArrayList list = null;
		if (count > 0) {
			list = dao.profileProductList(member_id, startRow, pageSize);
		}
		...
		// 페이지 이동
		forward.setPath("./fooding/khr/profile/productList.jsp");
