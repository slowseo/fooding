...
<link href="./fooding/khr/css/faqList.css" rel="stylesheet">
<!-- css 복사 -->
...
<div style="height: auto; min-height: 100%; padding-bottom: 41px;">
	...
	<tr>
		<td>${dto.bno }</td>
		<td>
		<a href="./SupportEventContent.sup?table_id=${dto.table_id }&pageNum=${requestScope.pageNum }">${dto.title }</a>
		</td>
	</tr>
	</c:forEach>
	</table>
	<c:if test="${!empty sessionScope.id && sessionScope.id.equals('admin') }">
	<button type="button" class="btn btn-secondary btn-sm" onclick=" location.href='./SupportEventForm.sup'; " style="float: right;">이벤트 작성</button>
	</c:if>
	
			<nav aria-label="Page navigation example">
  				<ul class="pagination justify-content-center">
  				<c:if test="${requestScope.startPage > requestScope.pageBlock }">
  				  <li class="page-item"><a class="page-link" href="./SupportEventList.sup?pageNum=${requestScope.startPage - requestScope.pageBlock }">이전</a></li>
				</c:if>
    			<c:forEach var="i" begin="${requestScope.startPage }" end="${requestScope.endPage }" step="1">
    			  <li class="page-item"><a class="page-link" href="./SupportEventList.sup?pageNum=${i }">${i }</a></li>
				</c:forEach>
				<c:if test="${requestScope.endPage < requestScope.pageCount }">
    			  <li class="page-item"><a class="page-link" href="./SupportEventList.sup?pageNum=${requestScope.startPage + requestScope.pageBlock }">다음</a></li>
				</c:if>		
  				</ul>
			</nav>
		<br>
