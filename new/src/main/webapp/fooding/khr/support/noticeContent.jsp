...
<link href="./fooding/khr/css/noticeContent.css" rel="stylesheet">
<!-- css 복사 -->
...
<section class="p-3 mb-2 bg-light text-dark">
	...
	<div class="div1">
		<button type="button" class="btn btn-primary btn-sm" onclick=" location.href='./SupportNoticeList.sup?pageNum=${requestScope.pageNum }'; ">목록으로</button>
	<c:if test="${!empty sessionScope.id && sessionScope.id.equals('admin') }">
		<button type="button" class="btn btn-secondary btn-sm" onclick=" location.href='./SupportNoticeUpdate.sup?table_id=${dto.table_id }&pageNum=${requestScope.pageNum }'; ">공지사항 수정</button>
		<button type="button" class="btn btn-danger btn-sm" onclick=" location.href='./SupportNoticeDelete.sup?table_id=${dto.table_id }&pageNum=${requestScope.pageNum }'; ">공지사항 삭제</button>
	</c:if>
	</div>
	<br>
	<br>	
