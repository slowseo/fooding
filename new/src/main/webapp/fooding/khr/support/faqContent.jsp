...
<section class="p-3 mb-2 bg-light text-dark">
	...
	<div class="div1">
		<button type="button" class="btn btn-primary btn-sm" onclick=" location.href='./SupportFaqList.sup?pageNum=${requestScope.pageNum }'; ">목록으로</button>
	<c:if test="${!empty sessionScope.id && sessionScope.id.equals('admin') }">
		<button type="button" class="btn btn-secondary btn-sm" onclick=" location.href='./SupportFaqUpdate.sup?table_id=${dto.table_id }&pageNum=${requestScope.pageNum }'; ">FAQ 수정</button>
		<button type="button" class="btn btn-danger btn-sm" onclick=" location.href='./SupportFaqDelete.sup?table_id=${dto.table_id }&pageNum=${requestScope.pageNum }'; ">FAQ 삭제</button>
	</c:if>
	</div>
	<br>
	<br>	
